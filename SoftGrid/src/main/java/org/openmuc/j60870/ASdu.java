...
public class ASdu {
    ...
    public String toString() {
        ...
        StringBuilder builder = new StringBuilder("Type ID: " + typeId.getCode() + ", " + typeId + ", "
    ...
    public boolean isConfirmation(ASdu aSdu) {
        boolean match = aSdu.getCauseOfTransmission().name().endsWith("_CON");
        if (informationObjects != null && aSdu.getInformationObjects() != null) {
            for (int i = 0; i < informationObjects.length; i++) {
                match = match && aSdu.getInformationObjects()[i].equals(informationObjects[i]);
            }
        }
        return match && typeId.equals(aSdu.getTypeId())  &&
